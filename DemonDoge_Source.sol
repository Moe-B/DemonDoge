...
// part 1
contract FLIGHTTICKET is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FLIGHTTICKET: The dividend tracker already has that address");

        FLIGHTTICKETDividendTracker newDividendTracker = FLIGHTTICKETDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FLIGHTTICKET: The new dividend tracker must be owned by the FLIGHTTICKET token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FLIGHTTICKET: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FLIGHTTICKET: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FLIGHTTICKET: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FLIGHTTICKET: Cannot update gasForProcessing to same value");
...
contract FLIGHTTICKETDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public override {
        require(false, "FLIGHTTICKET_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FLIGHTTICKET contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "FLIGHTTICKET_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FLIGHTTICKET_Dividend_Tracker: Cannot update claimWait to same value");
